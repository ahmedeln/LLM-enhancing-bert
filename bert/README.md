...
## Experimental Enhancements (local modifications)

This repository has been extended with a set of experimental, efficiency-focused enhancements to the encoder-only Transformer (BERT) aimed at reducing computation and parameter count while retaining representational power.

- **Factorized token embeddings**: use a smaller `embedding_size` and project to `hidden_size` (ALBERT-style factorized embeddings).
- **Parameter sharing**: reuse a single Transformer block across layers by enabling `share_parameters=True` in `BertConfig`.
- **Local-window + global token attention**: limit attention to a neighborhood of size `window_size` around each token while keeping the first token (CLS) as a global token attending to and being attended by all tokens.
- **SwiGLU feed-forward**: optionally replace the intermediate dense layer with a SwiGLU variant (set `use_swiglu=True` in `BertConfig`).

Example: enable all enhancements with a compact model configuration in code:

```
from bert import modeling
cfg = modeling.BertConfig(vocab_size=30522,
                                                    hidden_size=512,
                                                    embedding_size=128,
...
These changes are experimental. If you plan to pre-train or fine-tune with these options, verify behavior and performance on your target tasks and hardware.
...
_all_ of the the tokens corresponding to a word at once. The overall masking
...
We uploaded a new multilingual model which does _not_ perform any normalization
...
first _unsupervised_, _deeply bidirectional_ system for pre-training NLP.

_Unsupervised_ means that BERT was trained using only a plain text corpus, which
...
end-to-end, it can be beneficial to obtained _pre-trained contextual
embeddings_, which are fixed contextual representations of each input token
...
because the input labels are _character_-based, and SQuAD paragraphs are often
...
2.  **Punctuation splitting**: Split _all_ punctuation characters on both sides
...
arbitrary text corpus. Note that this is _not_ the exact code that was used for
...
be a single model which includes _most_ of the languages which have a
...
All code _and_ models are released under the Apache 2.0 license. See the
